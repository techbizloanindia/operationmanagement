...
export default function CreditDashboard() {
  ...
  const getUserBranches = (user: any) => {
    ...
    if (user.assignedBranches && user.assignedBranches.length > 0) {
      ...
      if (validAssignedBranches.length > 0) {
        console.log('🏢 Credit User using assignedBranches:', validAssignedBranches);
    ...
    if (user.branch && user.branch.toLowerCase() === 'multiple') {
      console.log('🏢 Credit User has "Multiple" branch access - will see all queries');
    ...
    if (filteredBranches.length === 0) {
      console.log('🚫 Credit User has no valid branch assignments - will see no queries');
    } else {
  ...
  const setUserAssignedBranches = () => {
    const userBranches = getUserBranches(user);
    console.log('🏢 Credit User setting assigned branches:', userBranches);
