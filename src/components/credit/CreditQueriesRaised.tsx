...
import { FaArrowLeft, FaSync, FaSearch, FaClock, FaUser, FaComments, FaPaperPlane, FaBell, FaWifi, FaHistory } from 'react-icons/fa';
...
const fetchQueries = async (userBranches: string[] = [], appNoFilter?: string): Promise<Query[]> => {
  try {
    ...
    console.log('🏢 Credit Dashboard: User branches:', userBranches);
    const branchParam = createBranchParam(userBranches);
    const appNoParam = appNoFilter ? `&appNo=${encodeURIComponent(appNoFilter)}` : '';
    const apiUrl = `/api/queries?status=all&team=credit&includeBoth=true${branchParam}${appNoParam}`;
    console.log('🌐 Credit Dashboard: API URL:', apiUrl);
    
    // Fetch all queries (not just pending) so we can properly filter resolved ones on frontend
    const response = await fetch(apiUrl);
    const result = await response.json();
    
    console.log('📊 Credit Dashboard: API Response:', { 
    ...
    // SIMPLIFIED: API already filters for credit team, so use all returned data
    console.log(`🔍 Credit Dashboard: API returned ${result.data.length} queries for credit team`);
...
export default function CreditQueriesRaised({ searchAppNo }: CreditQueriesRaisedProps = {}) {
      ...
      const unsubscribe = queryUpdateService.subscribe('credit', (update) => {
        ...
        if (update.action === 'created' && (update.markedForTeam === 'credit' || update.markedForTeam === 'both')) {
          ...
          if (selectedQuery && selectedQuery.appNo === update.appNo && currentView === 'chat') {
            ...
            console.log(`💬 Credit: Reloading messages for queryId: ${queryIdToUse} due to real-time update`);
  ...
  // Auto-refresh removed - using manual refresh only
  ...
  const individualQueries = React.useMemo(() => {
    console.log('🔍 Credit Dashboard: Processing queries for display...');
    console.log('🔍 Credit Dashboard: Raw queries data:', queries);
    
    if (!queries || queries.length === 0) {
      console.log('🚫 Credit Dashboard: No queries data available');
      return [];
    }
    ...
    queries.forEach((queryGroup, groupIndex) => {
      ...
      if (!queryGroup.queries || queryGroup.queries.length === 0) {
        ...
        pendingQueries.push({
          ...
          sendTo: queryGroup.sendTo || [],
          submittedBy: queryGroup.submittedBy || 'Unknown',
          submittedAt: queryGroup.submittedAt || new Date().toISOString(),
          status: 'pending',
          branch: queryGroup.branch,
          branchCode: queryGroup.branchCode,
      ...
      queryGroup.queries.forEach((query, index) => {
        ...
        const numericId = Date.now() + pendingQueries.length;
    ...
    console.log(`📊 Credit Dashboard: Created ${pendingQueries.length} pending queries for display (excluded resolved)`);
  ...
  const filteredApplications = React.useMemo(() => {
    console.log('🔍 Credit Dashboard: Filtering applications...', {
    ...
    if (!queries || queries.length === 0) {
      console.log('🚫 Credit Dashboard: No queries available for filtering');
      return [];
    }
    
    const applications = Array.from(groupedQueries.keys());
    console.log('🔍 Credit Dashboard: Available applications:', applications);
    ...
    console.log('🔍 Credit Dashboard: Filtered applications:', filtered);
    return filtered;
  ...
  const handleSelectQuery = (query: Query) => {
    console.log(`🎯 CREDIT: Opening chat for query:`, {
      queryId: (query as any).queryId || query.id,
      id: query.id,
      appNo: query.appNo,
    ...
    setCurrentView('chat');
    // Use queryId for proper cross-team chat isolation - SAME AS SALES
    loadChatMessages((query as any).queryId || query.id);
  };
  ...
  // Handle opening chat for a specific query (simple like sales)
  ...
  // Load chat messages - SIMPLE LIKE SALES
  const loadChatMessages = async (queryId: string | number) => {
    try {
      console.log(`🔄 CREDIT: Loading chat messages for query ${queryId}`);
      ...
      if (result.success) {
        ...
        console.log(`📬 CREDIT: Loaded ${messages.length} messages for query ${queryId}`);
        
        // Transform messages to include proper flags for ChatDisplay - SAME AS SALES
        ...
        console.error('CREDIT: Failed to load chat messages:', result.error);
        showSuccessMessage('❌ Failed to load chat messages');
      }
    } catch (error) {
      console.error('CREDIT: Error loading chat messages:', error);
      showSuccessMessage('❌ Error loading chat messages');
    }
  };

  // Send message - SAME AS SALES
  ...
  return (
    <div className="h-full w-full bg-white overflow-hidden shadow-xl rounded-lg max-w-6xl mx-auto">
      ...
      {/* View 3: Chat/Remarks - SAME DESIGN AS SALES */}
      {currentView === 'chat' && selectedQuery && (
        <div className="flex flex-col h-full">
          ...
          <div className="p-4 border-b border-gray-200 flex-shrink-0">
            ...
            {/* Simple Customer/Branch Display in Chat Header - SAME AS SALES */}
            <SimpleCustomerBranchDisplay
          ...
          {/* Use the new ChatDisplay component - SAME AS SALES */}
          ...
          {/* Message Input - EXACT SAME AS SALES */}
